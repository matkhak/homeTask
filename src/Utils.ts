...
export class Utils {
    ...
    static isValidArrayLength(value: number): boolean {
        return Utils.checkValueRange(value, MIN_LENGTH_ARRAY, MAX_LENGTH_ARRAY, INVALID_RANGE_LENGTH)
    }

    static isValidArrayValue(value: number): boolean {
        return Utils.checkValueRange(value, MIN_VALUE_ARRAY, MAX_VALUE_ARRAY, INVALID_RANGE_VALUE)
    }

    static checkValueRange(value: number, min: number, max: number, text_error: string): boolean {
    ...
    static checkValueFromStdin(value: number, stdinArray: number[]): void {
        ...
        else if (stdinArray[0] !== undefined && stdinArray[stdinArray[0]] !== undefined && stdinArray[stdinArray[0] + 1] == undefined) {
            Utils.isValidArrayLength(value);
